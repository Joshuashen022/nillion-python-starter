...
async def main():
    ...
    new_secret2 = nillion.NadaValues(
        {
            "my_int2": nillion.SecretInteger(300),
        }
    )
    ...
    receipt_store = await get_quote_and_pay(
        client,
        nillion.Operation.store_values(new_secret2, ttl_days=5),
        payments_wallet,
        payments_client,
        cluster_id,
    )
    # Store a secret
    store_id2 = await client.store_values(
        cluster_id, new_secret2, permissions, receipt_store
    )
    ...
    computation_time_secrets = nillion.NadaValues({"my_int3": nillion.SecretInteger(10)})
